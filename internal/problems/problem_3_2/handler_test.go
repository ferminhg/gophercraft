...
func TestHandler_Returns200_WhenFixed(t *testing.T) {
	// Create a dummy callback server
	...
	for i := 0; i < 50; i++ {
		...
		req := httptest.NewRequest(http.MethodPost, "/scrape/webhook/notify", bytes.NewReader(body))
	...
	// If there's a huge delta, response bodies are leaking connections
...
func TestEdgeCase_BadURL(t *testing.T) {
	...
	payload := map[string]interface{}{
		"callback_url": "http://this-url-definitely-does-not-exist.local",
	}
	body, _ := json.Marshal(payload)
