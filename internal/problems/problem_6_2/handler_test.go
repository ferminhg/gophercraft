...
func TestHandler_Returns200_WhenFixed(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		...
		req := httptest.NewRequest(http.MethodPost, "/proxy/session/refresh", bytes.NewReader(body)).WithContext(ctx)
	...
	// Wait a moment for all pending refreshes to attempt to write to their channels
...
func TestEdgeCase_SuccessfulRefresh(t *testing.T) {
	handler := NewHandler()
	...
	req := httptest.NewRequest(http.MethodPost, "/proxy/session/refresh", bytes.NewReader(body))
	w := httptest.NewRecorder()

	handler.ServeHTTP(w, req)

	if w.Code != http.StatusOK {
		t.Errorf("Expected status 200 for successful refresh, got %d", w.Code)
	}
}
